...
static void *get_data (void * var, FILE * file, unsigned long offset, bfd_size_type size, bfd_size_type nmemb, const char * reason)
{
    ...
    if (sizeof (size_t) < sizeof (bfd_size_type)&& ( (bfd_size_type) ((size_t) size) != size || (bfd_size_type) ((size_t) nmemb) != nmemb))
    {
        if (reason)
    ...
    if (mvar == NULL)
    {
        ...
        {
            if (reason)
                error (_("Out of memory allocating 0x%" BFD_VMA_FMT "x bytes for %s\n"),
                       amt, reason);
            return NULL;
        }
...
static unsigned int print_symbol (int width, const char *symbol)
{
...
static int process_file_header (void)
{
    ...
    if (do_header)
    {
        ...
        printf (_("  Number of program headers:         %ld"), (long) elf_header.e_phnum);
        ...
        printf (_("  Number of section headers:         %ld"), (long) elf_header.e_shnum);
        if (section_headers != NULL && elf_header.e_shnum == SHN_UNDEF)
            printf (" (%ld)", (long) section_headers[0].sh_size);
        putc ('\n', stdout);
        printf (_("  Section header string table index: %ld"), (long) elf_header.e_shstrndx);
    ...
    {
        if (elf_header.e_phnum == PN_XNUM && section_headers[0].sh_info != 0)
